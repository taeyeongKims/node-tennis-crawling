window.onload = function () { buildCalendar(); }    // 웹 페이지가 로드되면 buildCalendar 실행
        ...
        const nowMonth = new Date();  // 현재 달을 페이지를 로드한 날의 달로 초기화
        const today = new Date();     // 페이지를 로드한 날짜를 저장
        const selectDay = new Date();
        selectDay.setDate(today.getDate() + 14);
        ...
        function buildCalendar() {
            ...
            for (let nowDay = firstDate; nowDay <= lastDate; nowDay.setDate(nowDay.getDate() + 1)) {   // day는 날짜를 저장하는 변수, 이번달 마지막날까지 증가시키며 반복  
                ...
                else if(nowDay > today && nowDay <= selectDay){               // 선택 가능 예약 날짜인 경우
                    nowColumn.className = "selectDay";
                    nowColumn.onclick = function () { choiceDate(this); }
                }
                else {                                   // 미래인 경우
